        ...
        if ($token != '' && $user_id != '') {
            ...
            if ($check_key['status'] == 'true') {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $exist = $this->db->get_where('tbl_xai_matching', ['user_id' => $this->put('user_id')])->row();
                    if (empty($exist)) {
                        $this->db->insert('tbl_xai_matching', $formArray);
                        $effected = $this->db->insert_id();
                    } else {
                        $this->db->update('tbl_xai_matching', $formArray, ['user_id' => $this->put('user_id')]);
 ...
 $user_id = $this->db->select('user_id')->get_where('myteams',['id'=>$team_id])->row('user_id');        
    ...
    //get checkin checkout status
       ...
       public function getXaiData_get($user_id = NULL) {
          ...
          $data = $this->db->select('a.* , b.name as industry_name , c.name as experience_name , d.name as skill_name , e.name as softskill_name')
        ...
        if ($token != '' && $user_id != '') {
            ...
            if ($check_key['status'] == 'true') {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $exist = $this->db->get_where('tbl_xai_matching', ['user_id' => $this->put('user_id') ,'team_id' => $this->put('team_id')])->row();
                    if (empty($exist)) {
                        $this->db->insert('tbl_xai_matching', $formArray);
                        $effected = $this->db->insert_id();
                    } else {
...
//                    print_r($formArray);   die;
                    $exist = $this->db->get_where('tbl_xai_matching', ['user_id' => $user_id ,'team_id' =>$team_id])->row();
                    if (empty($exist)) {
                        $this->db->insert('tbl_xai_matching', $formArray);
                        $effected = $this->db->insert_id();
                    } else {
    ...
    //get xai data
       public function getXaiDataNew_get($user_id = NULL , $team_id = NULL) {
            $tokenid = $this->input->get_request_header('Secret-Key');
            $check_key = $this->authentication($user_id , $tokenid);  
            $data = $this->db->select('a.* , b.name as industry_name , c.name as experience_name , d.name as skill_name , e.name as softskill_name')
                 ->from('tbl_xai_matching as a')
                 ...
                 ->join('tbl_skill as d' ,'d.id = a.skill_id','left')
                 ->join('tbl_softskill as e' ,'e.id = a.softskills_id','left')
                 ->where(['a.user_id'=>$user_id ,'team_id'=>$team_id])->get()->row(); 
        if ($data) {
            $factorsData = [];
            $factors = $this->db->where_in('id',explode(',',$data->factors))->get('tbl_factors')->result();
            if($factors)
            ...
            if ($check_key['status'] == 'true') {
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $formArray = [
                        ...
                        'options_preferences' => $options_preferences,
                        'covid_vaccination_proof' => $covid_vaccination_proof,
                        'negative_tests_proof' => $negative_tests_proof,1
                        'none' => $none,
                    ];
